...
print('\nRelated:')
for num in range(len(related_1)):
    cos_val = cos_sim(nlp(related_1[num]).vector, nlp(related_2[num]).vector)
    #print('cos(%s, %s)=%f' % (related_1[num], related_2[num], cos_val))
    related_data = np.append(related_data, cos_val)
    
#for num in range(len(related_1)):
#    print('%f' % (cos_sim(nlp(related_1[num]).vector, nlp(related_2[num]).vector)))
    
print(related_data)

print('\nUnRelated:')
for num in range(len(related_1)):
    cos_val = cos_sim(nlp(unrelated_1[num]).vector, nlp(unrelated_2[num]).vector)
    #print('cos(%s, %s)=%f' % (unrelated_1[num], unrelated_2[num], cos_val))
    unrelated_data = np.append(unrelated_data, cos_val)
    
#for num in range(len(related_1)):
#    print('%f' % (cos_sim(nlp(unrelated_1[num]).vector, nlp(unrelated_2[num]).vector)))
...
fig.tight_layout()
plt.savefig('pairs-chwilla1995-SPACY-'+pretrained_model+'.png')
plt.show()

print(stats.ttest_ind(related_data,unrelated_data))
with open('ttest_ind-SPACY-'+pretrained_model+'.csv', 'w') as f_ttest_ind:
...
# Show the graph
# plt.savefig('ChwillaPairsHistogram-SPACY-'+pretrained_model+'.png')
plt.show()
...
# Show the graph
plt.savefig('ChwillaPairsBOXandHistogram-SPACY-'+pretrained_model+'.png')
plt.show()
