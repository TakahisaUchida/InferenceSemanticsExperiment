...
for i in range(N):
    ...
    for num in range(len(discourse_words_2)):
        if num == 0:
            if discourse_words_2[num] in nlp:
                discourse_vector_2 = nlp[discourse_words_2[num]]
        else:
            if discourse_words_2[num] in nlp:    
                discourse_vector_2 = (num * discourse_vector_2 + nlp[discourse_words_2[num]]) / (num + 1)
...
fig.tight_layout()
plt.savefig('McKoon-86-avg-'+pretrained_model+'.png')
plt.show()
