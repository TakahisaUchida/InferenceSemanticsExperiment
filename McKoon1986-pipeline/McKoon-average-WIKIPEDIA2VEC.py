...
#wiki2vec = Wikipedia2Vec.load('../pretrainedData/enwiki_20180420_100d.pkl')
...
f = open('McKoon_experiment-'+pretrained_model+'.csv', 'w')
...
for i in range(N):
    ...
    for num in range(len(discourse_words_2)):
        ...
        if num == 0:
            discourse_vector_2 = wiki2vec.get_word_vector(discourse_words_2[num])
        else:
            discourse_vector_2 = (num * discourse_vector_2 + wiki2vec.get_word_vector(discourse_words_2[num])) / (num + 1)
...
fig.tight_layout()
plt.savefig('McKoon-86-'+pretrained_model+'.png')
plt.show()
