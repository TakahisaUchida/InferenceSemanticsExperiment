...
#nlp = spacy.load("../pretrainedData/en_core_web_lg-2.3.1")

#pretrained_model = "en_core_web_lg"
#pretrained_model = "en_core_web_md"
pretrained_model = "en_core_web_sm"
nlp = spacy.load("../pretrainedData/" + pretrained_model + "-2.3.1")
...
vectors = np.empty([N,96])

#vectors = np.empty([N,50])

#f = open('metusalem2012_experiment-en_core_web_lg.csv', 'w')
...
for i in range(N):
    ...
    target_word_2_vector = nlp(target_word_2).vector
    target_word_3_vector = nlp(target_word_3).vector
 ...
 #   target_word_3_vector = wiki2vec.get_word_vector(target_word_3)
    ...
    for num in range(len(discourse_words_1and2)):
        ...
        if num == 0:
            discourse_vector_1and2 = nlp(discourse_words_1and2[num]).vector
        else:
            discourse_vector_1and2 = (num * discourse_vector_1and2 + nlp(discourse_words_1and2[num]).vector) / (num + 1)
