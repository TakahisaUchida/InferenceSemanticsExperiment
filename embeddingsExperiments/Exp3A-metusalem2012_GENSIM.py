...
# new for word2vec and pretrained
import gensim
...
# vector = nlp('state').vector

def cos_sim(v1, v2):
	return np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))

# wiki2vec = Wikipedia2Vec.load('../enwiki_20180420_100d.pkl')
#nlp = spacy.load('glove_6B_50d_twitter')
#nlp = spacy.load('glove_6B_100d')
#nlp = spacy.load("en_core_web_sm")
#nlp = gensim.models.KeyedVectors.load_word2vec_format('../pretrainedData/GoogleNews-vectors-negative300.bin', binary=True) #without *norm_only* param
...
#vectors = np.empty([N,300])
#vectors = np.empty([N,50])
#vectors = np.empty([N,100])
vectors = np.empty([N,200])
#vectors = np.empty([N,25])

f = open('metusalem2012_experiment-GENSIM-'+pretrained_model+'.csv', 'w')
...
for i in range(N):
    ...
    target_word_1_vector = nlp[target_word_1]
    target_word_2_vector = nlp[target_word_2]
    target_word_3_vector = nlp[target_word_3]
 ...
 #   target_word_3_vector = wiki2vec.get_word_vector(target_word_3)
    ...
    for num in range(len(discourse_words_1)):
        ...
        if num == 0:
            if discourse_words_1[num] in nlp:
                discourse_vector_1 = nlp[discourse_words_1[num]]
        else:
            if discourse_words_1[num] in nlp:    
                discourse_vector_1 = (num * discourse_vector_1 + nlp[discourse_words_1[num]]) / (num + 1)
    ...
    for num in range(len(discourse_words_1and2)):
        ...
        if num == 0:
            if discourse_words_1and2[num] in nlp:
                discourse_vector_1and2 = nlp[discourse_words_1and2[num]]
        else:
            if discourse_words_1and2[num] in nlp:    
                discourse_vector_1and2 = (num * discourse_vector_1and2 + nlp[discourse_words_1and2[num]]) / (num + 1)
...
ax.set_xticklabels(['Sent-Expect', 'Sent-Relat', 'Sent-Unrel', 'Disc-Expect', 'Disc-Relat', 'Disc-Unrel'])
fig.tight_layout()
plt.savefig('Metusalem-72-avg-GENSIM-'+pretrained_model+'.png')
plt.show()
...
plt.savefig('Metusalem-72-avg-distributions-GENSIM-'+pretrained_model+'.png')
plt.show()
